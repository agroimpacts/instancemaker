...
def process_one_tile(polygons_path: str, tile_row: Dict, out_dir: str, res_deg: float, nodata: float,
                      all_touched: bool, attrs: List[str]) -> str:
    try:
        ...
        if cog_tif.exists():
            return str(cog_tif)  # Skip existing tile


        band_names = [FIRST_BAND] + attrs
        profile = dict(driver="GTiff", height=height, width=width, count=len(band_names),
        ...
        with rasterio.open(tmp_tif, "w", **profile) as dst:
            dst.write(np.full((height, width), float(tile_id), dtype="float32"), 1)
            ...
            for bidx, col in enumerate(attrs, start=2):
                ...
                if not sel.empty:
                    shapes = [(g, float(v) if np.isfinite(v) else nodata)
                              for g, v in zip(sel.geometry, sel[col].to_numpy())]
                    band = rasterize(
        ...
        rio_copy(tmp_tif, cog_tif, driver="COG", compress="DEFLATE", predictor=2,
                 blocksize=512, bigtiff="IF_SAFER", overview_resampling="NEAREST",
...
def run_rasterization(polygons_path, tiles_path, out_dir, workers, res_deg, nodata, all_touched, attributes):
    ...
    for i in range(len(all_rows) // chunk_size + int(len(all_rows) % chunk_size != 0)):
        ...
        with ProcessPoolExecutor(max_workers=workers) as ex:
            futs = [
                ex.submit(
                    process_one_tile,
                    polygons_path, row, out_dir, res_deg,
                    nodata, all_touched, attrs
                )
                for row in chunk
            ]
        ...
        print(f"[chunk {i+1}] Done. {len(ok)} COGs written. Errors: {len(errs)}")
        for e in errs:
            print(e)
